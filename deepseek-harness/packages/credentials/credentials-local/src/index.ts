...
export class LocalCredentialProvider extends CredentialProvider {
  ...
  private async loadInitial(): Promise<void> {
    ...
    try {
      ...
      // 凭据文档不可信时降级为空凭据并记录，而不是让整个 dsh 启动失败：
      // 用户仍可进入应用重新配置；下次写入会基于空文档重建合法文件（自愈）。
      // 错误信息只进日志，绝不把凭据值打印出来。
